# api.py
# FastAPI backend: upload (/analyze), URL (/analyze-url), retro-compat (/predict), /healthz.
# Integra: ffprobe meta + analyzers (video/audio/heuristics_v2/forensic) + fusion.
# Aggiunge diskcache per download (httpx/yt-dlp) senza rimuovere funzionalità esistenti.
...
# ---- App ----
app = FastAPI(title="AI-Video Detector API", version="1.1.3")
...
def _is_html(content_type: str, text_head: bytes) -> bool:
    ...
    if "text/html" in ct:
        return True
    head = (text_head or b"")[:2048].strip().lower()
...
def _ffprobe_summary(path: str) -> Dict[str, Any]:
    """Ritorna meta essenziali (width/height/fps/duration/bit_rate, codecs...)."""
    try:
        ...
        fmt = data.get("format", {})
        width = height = fps = duration = bit_rate = 0.0
        vcodec = acodec = None
...
def _tmpfile(suffix: str) -> str:
    d = tempfile.mkdtemp(prefix="aiv_")
...
async def _download_via_ytdlp(url: str) -> Dict[str, Any]:
    ...
    dst = _tmpfile(".mp4")
    # yt-dlp best mp4, senza cookie (pubblico); se protetto → 422 con hint
    cmd = [
        "yt-dlp",
        "-f", "bv*+ba/b",
        "-o", dst,
    ...
    if p.returncode != 0 or not os.path.exists(dst) or os.path.getsize(dst) == 0:
        return {
            ...
            "hints": {
                ...
                "advice": "Se il link è protetto/rate-limited, usa 'Carica file' o 'Registra 10s e carica'."
            }
        }
    if cache:
        cache.set(key, dst, expire=7200)
...
def _build_response(meta: dict, fusion_out: dict) -> Dict[str, Any]:
    return {
        "ok": True,
        "meta": meta,
        **fusion_out
...
def _analyze_file(path: str, source_url: Optional[str] = None, platform: Optional[str] = None) -> Dict[str, Any]:
    ...
    # 5) Meta arricchito (compat con UI esistente)
    meta_block = {
    ...
    # 6) Restituzione
    out = _build_response(meta_block, fusion_out)
    out["forensic"] = forensic  # compat
    out["hints"] = hints        # utile per debug/UI
    out["video"] = vstats       # manteniamo per backward-compat e diagnosi
    out["audio"] = astats
    return out
...
async def analyze(file: UploadFile = File(None), url: Optional[str] = Form(None)):
    """
    Upload file (preferred) oppure fallback con url via form-data (compat), ma per URL usare /analyze-url.
    """
...
async def analyze_url(url: Optional[str] = Form(None), q: Optional[str] = Form(None),
    ...
    Usa yt-dlp se abilitato, fallback httpx; blocca HTML/HLS.
    """
    ...
    if USE_YTDLP:
        ...
        if y.get("ok"):
            ...
            # Alcuni casi protetti → messaggio esplicito
    ...
    if via is None:
        ...
        if not h.get("ok"):
            ...
            raise HTTPException(415, detail={"error": f"Unsupported or empty media: {err}"})
        via = h
...
async def predict(file: UploadFile = File(None),
    ...
    """
    Retro-compat: accetta sia file sia url|link|q. Reindirizza a /analyze o /analyze-url.
    """
    if file and file.filename:
    ...
    raise HTTPException(415, detail={"error": "Unsupported or empty media: no audio/video streams found",
                                     "ffprobe_found": True,
